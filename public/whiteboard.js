const roomId = document.body.dataset.room || window.WHITEBOARD_ROOM;
if (!roomId) {
...
state.selected = null; // { index, offsetX, offsetY }
state.undoStack = [];
state.redoStack = [];
...
function pointerToCanvas(evt) {
  ...
  return {
    ...
    pressure: evt.pressure && evt.pressure > 0 ? evt.pressure : 1
  };
}
...
function moveStroke(evt) {
  ...
  // dragging selection
  if (state.tool === 'select' && state.dragging && typeof state.selected?.index === 'number') {
    const idx = state.selected.index;
    const it = state.model[idx];
    if (!it) return;
    const dx = p.x - (state.dragLast && state.dragLast.x || p.x);
    const dy = p.y - (state.dragLast && state.dragLast.y || p.y);
    // apply delta
    if (it.type === 'stroke') {
      for (let pt of it.points) { pt.x += dx; pt.y += dy; }
    } else if (it.type === 'shape') {
      it.shape.x1 += dx; it.shape.x2 += dx; it.shape.y1 += dy; it.shape.y2 += dy;
    } else if (it.type === 'text') {
      it.text.x += dx; it.text.y += dy;
    }
    state.dragLast = { x: p.x, y: p.y };
    redrawAll();
    drawSelectionBox(it);
...
function endStroke(evt) {
  ...
  if (state.tool === 'select' && state.dragging) {
    const idx = state.selected && state.selected.index;
    if (typeof idx === 'number') {
      const it = state.model[idx];
      if (it) {
        // emit the updated full object to sync
        if (it.type === 'stroke') {
          socket.emit('whiteboard:draw', { roomId, stroke: { id: it.id, points: it.points } });
        } else if (it.type === 'shape') {
          socket.emit('whiteboard:draw', { roomId, stroke: { id: it.id, shape: it.shape } });
        } else if (it.type === 'text') {
          socket.emit('whiteboard:draw', { roomId, stroke: { id: it.id, text: it.text } });
        }
      }
    }
...
function bindSocketEvents() {
  ...
  socket.on('whiteboard:clear', () => clearCanvas(true));
}
